...
import { renderMatrixTraining, renderMatrixCheck, renderMatrixDemonstration, displayGraph } from "./print-graph.js";
import { showErrorCreateGraphMessage } from "./messages.js";
...
export const Router = {
    ...
    allLogic() {        
        ...
        const originalGraph = AppState.graph;
        this.answerGraph = new Graph(originalGraph.size, 0, originalGraph.genType);
        this.answerGraph.matrix = JSON.parse(JSON.stringify(originalGraph.matrix));
        ...
        this.newGraph = new Graph(originalGraph.size, 0, originalGraph.genType);
        for (let i = 0; i < originalGraph.size; i++){
            for (let j = 0; j < originalGraph.size; j++){
                this.newGraph.matrix[i][j] = -1;
            }
        }
